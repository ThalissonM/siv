        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- adicionar aqui a foto do modelo do veículo --}}
                ...
                <form action="{{ url('modelos_veiculos/store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded px-8 pt-6 pb-8 mb-4 flex items-center" style="padding:10px;">
                    @csrf
                    <input type="hidden" value="{{$modelo->id}}" id="id" name="id">
                    <div class="grid gap-6 mb-6 md:grid-cols-1 w-full">
                        ...
                        <figure class="max-w-lg mx-auto text-center"> <!-- Added mx-auto and text-center classes -->
                            <img class="h-auto max-w-full rounded-lg" src="{{asset($modelo->image)}}" alt="Imagem">
                        ...
                        <div>
                            ...
                            <input type="text" id="modelo" name="modelo" value="{{$modelo->modelo}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="uvs" required>
                        </div>
            ...
            @livewire('pergunta-component', ['modelo_veiculo_id' => $modelo->id])
            </div>
