            ...
            <div class="grid gap-1 mb-1 md:grid-cols-1 w-full">
                ...
                <form action="{{ url('inspecoes/store') }}" enctype="multipart/form-data" method="POST" class="bg-white rounded px-8 pt-6 pb-8 mb-4 flex items-center" style="padding:10px;">
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-1 w-full">
                        ...
                        <input type="hidden" value="{{$inspecao->id}}" id="id" name="id">
                        @endif
                        <input type="hidden" value="{{$modelo->id}}" id="modelo_veiculo_id" name="modelo_veiculo_id">



                        
                        <label for="veiculo_id" class="block mb-2 text-sm font-medium text-gray-900">Veículo ou Maquinário</label>
                        <select id="veiculo_id" name="veiculo_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            @if(!isset($veiculos))
                        ...
                        <label for="motorista" class="block mb-2 text-sm font-medium text-gray-900 ">Motorista</label>
                            <select id="motorista_id" name="motorista_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        <div>
                            ...
                            <input type="text" id="uvs" name="uvs" value="{{ isset($inspecao) ? $inspecao->uvs : 'Viasolo Montes Claros' }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="uvs" required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="date" id="data" name="data" value="{{ isset($inspecao) ? \Carbon\Carbon::parse($inspecao->data)->format('Y-m-d') : date('Y-m-d') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required>
                        </div>
                        <div>
                            <label for="horario" class="block mb-2 text-sm font-medium text-gray-900 ">Hora</label>
                            <input type="time" id="horario" name="horario" value="{{ isset($inspecao) ? \Carbon\Carbon::parse($inspecao->data)->format('H:i') : date('H:i') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required>
                        </div>
                        <div>
                            <label for="turno" class="block mb-2 text-sm font-medium text-gray-900">Turno</label>
                            <select id="turno" name="turno" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        <div class="grid gap-6 mb-6 md:grid-cols-1">
                            ...
                            @foreach ($perguntas as $key => $pergunta)
                                ...
                                <ul class="items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex">
                                    ...
                                    <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r">
                                        <div class="flex items-center ps-3">
                                            <input id="pergunta-{{ $pergunta->id }}-nao" type="radio" value="nao" name="pergunta[{{ $pergunta->id }}][radio]" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500" {{ isset($pergunta->resposta) && $pergunta->resposta == 'nao' ? 'checked' : '' }}>
                                            <label for="pergunta-{{ $pergunta->id }}-nao" class="w-full py-3 ms-2 text-sm font-medium text-gray-900">Não OK</label>
                                        </div>
                                    </li>
                                    <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r">
                                        <div class="flex items-center ps-3">
                                            <input id="pergunta-{{ $pergunta->id }}-na" type="radio" value="na" name="pergunta[{{ $pergunta->id }}][radio]" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500" {{ isset($pergunta->resposta) && $pergunta->resposta == 'na' ? 'checked' : '' }}>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="justificativa-{{ $pergunta->id }}" name="pergunta[{{ $pergunta->id }}][justificativa]" value="{{ isset($pergunta->justificativa) ? $pergunta->justificativa : '' }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="especificar caso não ok">
                                </div>
                            @endforeach
